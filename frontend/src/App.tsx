...
export function App() {
  ...
  useEffect(() => {
    if (isShareModalOpen && shareUrl && qrCanvasRef.current) {
      // Small timeout to ensure DOM element is mounted and sized properly
  ...
  const expenseChartData = useMemo(() => {
    // Basic grouping by day for the chart
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="bg-gradient-to-br from-teal-500 to-teal-700 border-none shadow-xl shadow-teal-500/20 text-white flex flex-col justify-between rounded-2xl">
          <CardContent className="p-6 h-full flex flex-col justify-between">
            ...
            <div className="mt-4">
              <button onClick={() => setActiveTab('meals')} className="text-sm bg-white/20 hover:bg-white/30 transition-colors py-2 px-4 rounded-lg w-full text-left flex justify-between items-center backdrop-blur-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="bg-white/60 backdrop-blur-md border border-white/40 rounded-2xl shadow-sm p-6 lg:col-span-2 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setActiveTab('expenses')} className="text-teal-600 text-sm font-medium hover:underline">View Ledger</button>
          </div>
        ...
        <Card className="bg-white/60 backdrop-blur-md border border-white/40 rounded-2xl shadow-sm p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setActiveTab('members')} className="text-teal-600 text-sm font-medium hover:underline">View All</button>
          </div>
                ...
                <td className="p-4 text-right">
                  {isAdmin && (
                    <Button 
                      ...
                      className={`text-xs rounded-lg transition-colors ${member.is_active ? 'text-red-600 hover:bg-red-50 hover:text-red-700 border-red-200' : 'text-teal-600 hover:bg-teal-50 hover:text-teal-700 border-teal-200'}`}
                    >
  ...
  const MealsView = () => {
    // Generate dates around the selected mealDate
    const dates = Array.from({ length: 15 }, (_, i) => format(new Date(new Date(mealDate).getTime() + (i - 7) * 86400000), "yyyy-MM-dd"));
  ...
  return (
    <div className="min-h-screen bg-[#e2f1f0] text-slate-800 font-sans flex overflow-hidden selection:bg-teal-200 relative">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden z-10">
        <header className="h-20 bg-white/20 backdrop-blur-md border-b border-white/30 px-8 flex items-center justify-between sticky top-0">
          <div>
